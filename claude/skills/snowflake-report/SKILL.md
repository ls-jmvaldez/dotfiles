...
| Classic iSeries prod replicas (payment attempts/declines, drafts, …) | — | `SYSTEM_I.ANALYTICS_DM."Payment_Transactions_CRCDPASSD"` (108M rows, 2018→now; also `_CRCDETC`, pre-built `_Denial_Rate*` views) | Table and column identifiers are mixed/lower-case — **double-quote everything**. Replicas are column-pruned: CRCDPASSD keeps `crcdresp`/`crcdmsg2`/`crcduser`/`charged_date`/`crcdcust` but drops CRCDRSPCD, CRCDAVSCD, CRCDMSG1 — response-code and AVS questions can't be answered here, only message-text/volume/writer ones. atlas-* microservice stores (e.g. transactions-service) are not replicated at all. |
