...
If the arguments start with a project flag (e.g. `--opsuc`, `--coreapp1`), use the matching project profile from the knowledge file. The flag is case-insensitive and maps to the project key:

- `--opsuc` → OPSUC (Gold Diggers)
- `--coreapp1` → COREAPP1 (Internal Tools)

If no flag is provided, resolve the default project using `$JIRA_PROJECT` env var, or fall back to the Default Project setting in the knowledge file.
...
3. Look up the matching project profile in the knowledge file for board ID, issue type IDs, and sprint IDs
