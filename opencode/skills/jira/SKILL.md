...
When composing issue descriptions (stories, subtasks, epics, comments), follow the `writer` skill using **The Engineer** persona. Key rules:
...
For the full style guide, load the `writer` skill.
...
All requests use the `$JIRA_AUTH_TOKEN` env var — a base64-encoded string of `email:api-token`.
...
**Before any Jira operation**, verify auth is available:

```bash
if [ -z "$JIRA_AUTH_TOKEN" ]; then
  echo "ERROR: JIRA_AUTH_TOKEN not set. See setup options below."
  exit 1
fi
```
...
Choose whichever method fits your tooling.

#### Option A: Inline with 1Password CLI

Resolve the secret at launch time:

```bash
JIRA_AUTH_TOKEN="$(op read 'op://Vault/Item/credential')" opencode
```

#### Option B: Shell alias or export

For users without a password manager. Export the pre-computed base64 token before launching:

```bash
export JIRA_AUTH_TOKEN="dXNlckBleGFtcGxlLmNvbTpteS1hcGktdG9rZW4="
opencode
```

Or add it to your shell profile (`.zshrc`, `.bashrc`, etc.). Be aware this stores the token in plaintext.

#### Option C: File reference in OpenCode config

Store the base64 token in a file and reference it via OpenCode's `{file:...}` config variable syntax. Useful for keeping secrets out of shell history:

```bash
echo -n "dXNlckBleGFtcGxlLmNvbTpteS1hcGktdG9rZW4=" > ~/.secrets/jira-token
chmod 600 ~/.secrets/jira-token
```

Then in `opencode.json`, you can reference it with `{file:~/.secrets/jira-token}` in any config value that supports variable substitution.

#### Option D: OpenCode plugin (shell.env hook)

Inject the token into every shell execution automatically via a plugin:

```javascript
// ~/.config/opencode/plugins/jira-env.js
export const JiraEnvPlugin = async () => {
  return {
    "shell.env": async (input, output) => {
      output.env.JIRA_AUTH_TOKEN = process.env.JIRA_AUTH_TOKEN;
    },
  };
};
```

This is useful when the token is set in the parent process but needs to be available inside OpenCode's shell tool.
...
Switch teams without editing the skill or restarting:
...
JIRA_PROJECT=OPSUC opencode

# Or combine with 1Password
JIRA_AUTH_TOKEN="$(op read 'op://Vault/Item/credential')" JIRA_PROJECT=OPSUC opencode
```
