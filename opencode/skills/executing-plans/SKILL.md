...
**Track progress with tasks:** Use the TodoWrite tool to create tasks for each major work item from the plan. Update status as work progresses (`in_progress` when starting, `completed` when done). This makes execution visible to the user and persists across context compactions.
